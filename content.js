...
(async function init() {
    ...
    // Storage変更監視（Popupで保存した直後に反映させるため）
    chrome.storage.onChanged.addListener((changes, namespace) => {
        if (namespace === 'local' && changes.lazy_entry_profile) {
            MY_PROFILE = changes.lazy_entry_profile.newValue;
            console.log("Lazy Entry: Profile updated from storage.", MY_PROFILE);
        }
    });
})();
...
async function loadProfile() {
    try {
        const data = await chrome.storage.local.get('lazy_entry_profile');
        console.log("Loaded storage data raw:", data);

        if (data.lazy_entry_profile) {
            MY_PROFILE = data.lazy_entry_profile;
            console.log("Lazy Entry: MY_PROFILE Loaded:", MY_PROFILE);
        } else {
            console.warn("Lazy Entry: No profile data found. Please set it in the popup.");
            MY_PROFILE = null;
        }
    } catch (e) {
        console.warn("Lazy Entry: Failed to load profile. (Maybe context invalidated)", e);
    }
}
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    if (request.action === "autoFill") {
        ...
        (async () => {
            ...
            // 必須データの構造チェック (簡易)
            if (MY_PROFILE && MY_PROFILE.name) {
                try {
                    autoFill();
                    sendResponse({ status: "done" });
                } catch (e) {
                    console.error("Lazy Entry: autoFill error", e);
                    sendResponse({ status: "error", message: e.message });
                }
            } else {
                console.error("Lazy Entry: Invalid profile data structure.", MY_PROFILE);
                alert("プロフィールデータが不正か、読み込まれていません。Popupで再保存してください。");
                sendResponse({ status: "error", message: "Invalid profile" });
            }
        })();
        return true; // async for sendResponse
    }
});
...
function autoFill() {
    ...
    console.log("Lazy Entry: Auto fill started with profile:", MY_PROFILE);
    ...
    inputs.forEach(el => {
        ...
        if (txt.includes('city') || txt.includes('g1') || txt.includes('shiku') || txt.includes('adrs1') || txt.includes('gadrs1') || txt.includes('kadrs1')) {
            ...
            const city = MY_PROFILE.address?.city || "";
            const street = MY_PROFILE.address?.street || "";
            fillField(el, city + street); // 連結して入れるパターンが多い(gadrs1)
        ...
        if (txt.includes('mail') || txt.includes('account')) {
            const email = MY_PROFILE.email?.main || "";
            // Split email (account / domain)
            const emailParts = email.split('@');

            if ((txt.includes('account') || txt.includes('user')) && emailParts.length > 0) {
                fillField(el, emailParts[0]);
            } else if (txt.includes('domain') && emailParts.length > 1) {
        ...
        if ((txt.includes('bunri') || txt.includes('memo4')) && el.type === "radio") {
            ...
            // 今回のケースでは理系=10002
            const bunri = MY_PROFILE.school?.bunri;
            if (el.value === "10002" && bunri === "理系") {
                el.checked = true;
                fillField(el, "10002");
            }
            if (el.value === "10001" && bunri === "文系") {
                el.checked = true;
                fillField(el, "10001");
            }
        }
